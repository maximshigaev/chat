...
export const myProfileShape = {
    schema: object().shape({...signUpSchema, 
        jobTitle: string()
            .min(2, `Profession should be more than 2 characters`)
            .max(20, `Profession should be less than 20 characters`)
            .trim(),
        timeZone: string()
            .length(6, `Timezone should be exactly 6 characters`)
            .matches(/^(?:Z|[+-](?:2[0-3]|[01][0-9]):[0-5][0-9])$/, `Timezone doesn't match the format +04:00`)
            .trim(),
        fb: string()
            .min(6, `Facebook should be more than 6 characters`)
            .max(20, `Facebook should be less than 20 characters`)
            .matches(/^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/, `Invalid url`)
            .trim(),
        tw: string()
            .min(6, `Twitter should be more than 6 characters`)
            .max(20, `Twitter should be less than 20 characters`)
            .matches(/^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/, `Invalid url`)
            .trim(),
        inst: string()
            .min(6, `Instagram should be more than 6 characters`)
            .max(20, `Instagram should be less than 20 characters`)
            .matches(/^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/, `Invalid url`)
            .trim(),
        lkdn: string()
            .min(6, `Linkedin should be more than 6 characters`)
            .max(20, `Linkedin should be less than 20 characters`)
            .matches(/^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/, `Invalid url`)
            .trim(),
    }),
}
